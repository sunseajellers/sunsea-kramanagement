...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                          JEWELMATRIX SYSTEM ARCHITECTURE                      │
└─────────────────────────────────────────────────────────────────────────────┘
         ...
         │                               │                               │
    ┌────▼──────────────────┐   ┌────────▼────────────┐   ┌────────────▼─┐
    │  SERVICE LAYER        │   │  FIREBASE ADMIN SDK │   │   EXTERNAL   │
    │  (src/lib/*.ts)       │   │  (Server-side logic)│   │   SERVICES   │
    │                       │   │                     │   │              │
    │ ┌─────────────────┐   │   │ ┌──────────────┐   │   │ ┌──────────┐ │
    │ │ Task Services   │   │   │ │ Admin Auth   │   │   │ │ Email    │ │
    │ │ - taskService   │   │   │ │ Admin DB     │   │   │ │ Service  │ │
    │ │ - enhanced      │   │   │ │              │   │   │ └──────────┘ │
    │ │   TaskService   │   │   │ └──────────────┘   │   │              │
    │ └─────────────────┘   │   └────────┬───────────┘   │ ┌──────────┐ │
    │ ┌─────────────────┐   │            │               │ │ SMS API  │ │
    ...
    └───────────────────────┘
         ...
         ▼                    ▼
    ┌─────────────────┐  ┌────────────────────┐
    │ Context Layer   │  │ Firebase Auth      │
    │ (React Context) │  │ - User Auth        │
    │ - AuthContext   │  │ - Session Mgmt     │
    └─────────────────┘  └────────────────────┘
```
...
└──────────────────────────────────────────────────────────────────────────┘
              ...
              └─────┬─────┘              └────────┬────────┘
                ...
                │                 │                 │
         ┌──────▼──────┐   ┌──────▼──────┐   ┌─────▼──────┐
         │ Firestore   │   │ Admin SDK   │   │ External   │
         │ Read/Write  │   │ Operations  │   │ Services   │
         └──────┬──────┘   └──────┬──────┘   └─────┬──────┘
                │                 │                 │
         ┌──────▼──────────────────▼─────────────────▼──────┐
...
```
┌──────────────────────────────────────────────────────────────────────┐
│                  PERFORMANCE SCORING CALCULATION FLOW                 │
└──────────────────────────────────────────────────────────────────────┘
         ...
         │ (Only where assignedTo.includes(userId)) │
         └────┬─────────────────────────────────────┘
              │
         ┌────▼─────────────────────────────────────┐
         │ On-Time Tasks / Completed Tasks          │
         │ Check: completedAt <= dueDate/finalDate  │
         │ Linear decay for overdue tasks           │
         └────┬─────────────────────────────────────┘
              │
         ┌────▼─────────────────────────────────────┐
         │ Revision Penalties Applied               │
         │ -5% per revision on final score          │
         │ Cumulative for multiple revisions        │
         └────┬─────────────────────────────────────┘
              │
              ├────────────────┬────────────────┐
              │                │                │
         ┌────▼────┐    ┌──────▼─────┐   ┌─────▼────┐
         │ KRA      │    │Consistency │   │Weighted  │
         │Alignment │    │Bonus       │   │Intensity │
         │(if KRAs) │    │(90%+velo)  │   │(Priority)│
         └────┬────┘    └──────┬─────┘   └─────┬────┘
              │                │             │
              └────────────────┼─────────────┘
                               │
...
└──────────────────────────────────────────────────────────────────────┘
    ...
    └───┬────┘   └──────┬─────┘   └────┬────┘
        ...
        │ - Boolean: achieved/not        │
        └───────────────┬────────────────┘
                        │
        ┌───────────────▼────────────────┐
        │ Aggregate KR Progress          │
...
```
┌──────────────────────────────────────────────────────────────────────┐
│              MULTI-ASSIGNEE TASK MANAGEMENT FLOW                      │
└──────────────────────────────────────────────────────────────────────┘
        ...
        └───┬────┘       └───┬────┘      └───┬────┘
            ...
            │                │                │
        ┌───▼──────────────────────────────────▼────┐
        │ Each User Works on Task                    │
        │ (May have different progress)             │
        └───┬──────────────────────────────────────┘
            │
        ┌───▼──────────────────────────────────────┐
        │ Individual Completion Tracking            │
        │ - user1_status: in_progress              │
        │ - user2_status: completed                │
        │ - user3_status: pending_review           │
        └───┬──────────────────────────────────────┘
            │
        ┌───▼──────────────────────────────────────┐
        │ Task Status Resolution                    │
        │ ONLY when ALL assignees complete:        │
        │ status → completed                       │
        └───┬──────────────────────────────────────┘
            │
        ┌───▼──────────────────────────────────────┐
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Task Completion → NotificationService → Email + In-App Toast       │
│  Task Overdue → NotificationService → Email + Mobile Push           │
│  OKR Updated → NotificationService → Email + Dashboard Alert        │
│  Ticket Resolved → NotificationService → Email + SMS (Optional)     │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Cron Job (Daily/Hourly) → reminderService.processDueReminders()    │
│  ├─ Check due reminders                                             │
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Every Service Operation → activityLogger.log({...})                │
...
│  ├─ Task Analytics: Completion rates, cycle time                   │
│  ├─ Risk Detection: Overdue patterns, velocity drop                │
│  ├─ Anomaly Detection: Unusual task patterns                       │
│  ├─ Predictions: Task at-risk scoring                              │
...
│  ├─ On-demand: User clicks "Export to PDF"                         │
│  ├─ Scheduled: Weekly MIS reports via cron                         │
│  ├─ Format: CSV, PDF, JSON                                         │
│  └─ Storage: Firebase Cloud Storage + Download                     │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
│   ├── AdminOperationalActions.tsx
│   ├── AdminVerificationQueue.tsx
│   ├── ExtensionRequestQueue.tsx
│   ├── HolidayManager.tsx
...
│   ├── MainLayout.tsx
│   └── DashboardLayout.tsx
├── AdminLayout.tsx              # Root admin layout
...
Deploy scheduled tasks via Firebase Console → Cloud Functions → Create Function.
...
## 🧠 13. Deep Understanding: How JewelMatrix Works End-to-End
...
5. Manager clicks "Create Task" → API call to `/api/tasks/`

**Step 2: API Processing**
```
POST /api/tasks/
├─ Request received with auth token
├─ withAdmin() middleware validates authorization
├─ Input sanitization (sanitizeFormData)
├─ Zod validation against task schema
├─ Create task document in Firestore
│  ├─ Set status: 'assigned'
│  ├─ Set priority: 'critical'
│  ├─ Add assignedTo: ['employee_id']
│  ├─ Calculate initial score weight: 4x (critical)
│  └─ Timestamp: createdAt
├─ Log activity in admin_logs
├─ Trigger notificationService
│  ├─ Create notification for employee
│  ├─ Send email: "New Task Assigned"
...
1. Employee clicks task → Task detail view opens
2. Employee reads description and checklist
3. Clicks "Start Working" button → Updates task status
...
4. When all items complete → Task ready for review
...
│  └─ Completed Tasks / Total Tasks = 85%
...
│  ├─ No revisions on this task
│  ├─ Score: 100%
├─ Calculate KRA Alignment Score
│  ├─ Task linked to "Sales Execution" KRA
│  ├─ Check KRA assignment
│  └─ Score: 95%
├─ Apply Consistency Bonus
│  ├─ Check last 4 weeks velocity
│  ├─ 91% completion rate → Qualifies for bonus
│  └─ +5% bonus
├─ Weight formula: (30% × 85) + (30% × 92) + (20% × 100) + (10% × 95) + (10% × bonus)
├─ Overall Score = 90.7
├─ Store in user_scores collection
│  ├─ userId: 'employee_123'
│  ├─ score: 90.7
│  ├─ weekOf: 2026-01-27
│  └─ breakdown: { completion, timeliness, quality, kra, bonus }
├─ Check achievement thresholds
│  └─ Score > 90: Award "Star Performer" badge
├─ Trigger notifications
│  └─ "Great work! Your score increased to 90.7"
└─ Update dashboards in real-time (Firestore listeners)
```
...
├─ Key Result 1: "Reduce support ticket resolution time to 4 hours"
├─ Key Result 2: "Achieve 95% first-contact resolution rate"
└─ Key Result 3: "Increase customer NPS score to 70"
```
...
├─ Task "Process 50 tickets": 50 tickets resolved in 3.5 hours avg
├─ Task "Customer feedback": 45 ratings collected, avg 4.2/5 stars
└─ Metric auto-updates: KPI value = 4.2/5 status

Weekly Aggregation:
├─ 250 tickets processed (avg 3.8 hours resolution)
├─ 234 first-contact resolutions / 250 = 93.6% resolution rate
├─ Customer feedback aggregate: 4.3/5 = NPS trending up
└─ Key Result progress: (3.8/4.0) + (93.6/95) + (4.3/5) / 3 = ~94% progress

Quarterly Summary:
├─ Multiple weeks of data aggregated
├─ Objective progress = Average of all KR progress
├─ Result: 91% → "On track for completion"
└─ Adjustments: Can pivot tasks/KRs if needed
```
...
├─ userId: person who owns this resource
├─ departmentId: person's department
├─ teamId: person's team (optional)
└─ ownershipPath: hierarchical ownership for querying
...
→ Allows sub-collections to scale independently
→ Faster reads of main document
```
...
Time 0.2: Manager A marks verified → status: completed
Time 0.3: Manager B marks rejected → status: revision_requested
Result: Last write wins → Inconsistent state!
```
...
JewelMatrix uses cursor-based pagination rather than offset-based pagination. This design choice emerges from Firestore's inability to efficiently skip documents—there's no native "skip 100 documents" operation. Instead, the system remembers the last document read (the cursor) and starts the next query after that position. This means pages remain consistent even if data changes between requests, and pages never skip or duplicate records.
...
**The N+1 Query Problem**: A common mistake is fetching all tasks first, then for each task, fetching the assigned user's details. This creates one query to fetch tasks plus N additional queries to fetch users, scaling linearly with task count. JewelMatrix mitigates this through denormalization—the task document includes frequently-accessed user fields like name, email, and department, eliminating the need for the extra queries.
...
Pagination cursors represent a specific point in a sorted result set. Rather than being a page number, a cursor is opaque data that records the last document's ID and sort key. When requesting the next page, the system uses this cursor to start the query after that position. This approach handles data mutations gracefully—if documents are added or deleted between page requests, the cursor ensures the next page continues smoothly from where the previous page ended, with no gaps or duplicates.
...
Context itself is a client-side state mechanism—it doesn't persist across page reloads. To solve this, JewelMatrix hooks into Firebase's `onAuthStateChanged` listener, which persists authentication state across browser restarts. This means when a user closes and reopens the app, Firebase automatically recognizes them, re-fetches their profile, and restores the context.
...
Data that needs to persist across browser sessions must be stored on the server—this includes tasks, OKRs, user roles, and scores. Data that only matters for the current session can be client-side—current filter selections, form input values, UI animation states, or temporary calculations.
...
This happens for multiple concurrent users—if manager updates a task's status, every employee with that task open sees the update within milliseconds. This is the "real-time" aspect of JewelMatrix's architecture.
...
Manual cache invalidation happens explicitly—when a user adds a team member, the code calls `invalidate()` immediately, forcing the next access to fetch fresh data from Firestore rather than waiting for the TTL to expire.
...
Tasks in JewelMatrix flow through a specific state machine with well-defined allowed transitions. A task begins as "not_started," moves to "assigned" when given to a team member, transitions to "in_progress" when work begins, and eventually reaches "completed" after review. However, not every transition is valid—you cannot jump directly from "assigned" to "completed," and you cannot move backward from "completed" to "in_progress."

This state machine prevents workflow violations. A task cannot be marked completed without first being submitted for review ("pending_review" status). A completed task can only move to "archived" status. A cancelled task is terminal—it cannot be uncancelled. These rules are enforced at the server level, not just in the UI, ensuring no user can bypass them through API manipulation.

Tasks can also enter an "overdue" state when their due date passes without completion. This is a special state that doesn't require explicit user action—the system detects overdue tasks and marks them automatically. An overdue task must return to "in_progress" to continue work, enforcing visibility of late deliverables.
...
All score recalculations must be transactional—the task status update and score recalculation happen atomically, or not at all. This prevents inconsistent states where the task shows completed but the score hasn't updated yet.
...
This cascade means completing a task doesn't just affect your individual score—it cascades up through the entire strategy hierarchy. A sales representative completing a "close customer" task increments a KRA like "new customer acquisition," which moves toward a KR like "acquire 50 new customers," which contributes to an Objective like "grow revenue by 30%."

The cascade operates on summation—an Objective's progress is the average of its KRs' progress. A KR's progress is calculated from its target value versus current value. Different KR types (percentage complete, number of units, currency, boolean yes/no) have different progress calculation formulas.
...
JewelMatrix applies strict listener scoping—listeners always include where clauses limiting them to relevant data. A user's task list listener only watches tasks assigned to that user, not the entire task collection. This keeps listener costs proportional to data volume actually affecting the user.
...
Client-provided timestamps (due dates) are different—users choose when tasks should be done. These are validated on the server (due dates must be in the future) but stored as-is. System-generated timestamps like createdAt and completedAt always use server timestamps.
...
**Business logic testing** specifically validates the TaskBusinessRules class. Valid status transitions (assigned → in_progress) should pass. Invalid transitions (completed → assigned) should fail. Attempting to mark a task complete without completing all checklists should fail. These tests ensure the state machine and validation rules work correctly.
...
JewelMatrix's business rules are critical—they enforce constraints that prevent invalid data. Testing ensures rules are enforced correctly. The system tests that:
...
**Layer 1: Client Side** — Check if the update request was even sent. Browser console logs reveal whether the JavaScript code attempted the update. DevTools Network tab shows the HTTP request: Was it sent? What status code was returned? What was in the request body and response?

**Layer 2: API Route** — The API route receives the request, validates it, and initiates the Firestore update. Checking API logs shows: Did auth validation pass? Was the status value valid? Did the task exist? Did the status transition pass business rule validation?

**Layer 3: Firestore Database** — The actual database layer. Firebase Console shows whether the task document was updated: Is the status field actually changed? Is the updatedAt timestamp recent? This confirms whether the database update happened.

**Layer 4: Activity Logs** — JewelMatrix logs all state-changing operations to admin_logs. Querying this collection for the task shows: Was a task_status_updated action logged? What was the timestamp? What was the old vs new status?

**Layer 5: Real-time Listeners** — If the database has the new status but the user's screen doesn't reflect it, the issue is the listener. Maybe the listener unsubscribed, or maybe a JavaScript error prevents state updates. Checking listener debug logs reveals whether snapshots are being received.

**Layer 6: Browser Storage** — LocalStorage contains auth tokens. If the token is expired, requests fail silently. Checking LocalStorage reveals whether a valid token exists.

This systematic approach eliminates confusion about where the problem lives. Problems are either in frontend code, API validation, database operation, logging, real-time synchronization, or authentication—narrowing the diagnosis through methodical layers.
...
**"Invalid field path"** occurs when referencing a field that doesn't exist. Firestore is strict about field names—a typo means the query fails. Using dot notation for nested objects requires the parent field to exist.
...
**Performance metrics** measure query speed and API responsiveness. Average query time shows how long typical database reads take. 95th and 99th percentile response times reveal worst-case performance—if 99th percentile is slow, some users experience poor performance. These metrics immediately surface database bottlenecks (missing indexes) or code inefficiencies.

**Reliability metrics** track success vs failure rates. Error rate measures what percentage of requests fail. Task failure rate might track tasks that failed validation or couldn't be created. Monitoring these reveals systemic problems—if error rate suddenly jumps, something broke.
...
Errors occur in production despite testing. JewelMatrix captures errors with context—when an error happens, the system also records what the user was doing, which user encountered it, which system component failed. This context is crucial for debugging.
...
**Synchronous Direct Calls**: One service calls another directly and waits for the response. When assigning a task to an employee, the task service directly calls the user service to verify the employee exists and is active. This ensures immediate feedback—if the employee doesn't exist, the assignment fails immediately. The downside is tight coupling: the task service depends on the user service being available. If the user service is slow or down, task assignment becomes slow or fails.
...
**Transaction-Based Atomic Operations**: Multiple updates must happen atomically—all succeed or all fail. Transferring a task from one user to another involves: deducting points from the old user's score, adding points to the new user's score, updating the task's assignee, and logging the activity. If any of these fails partway through, the system enters an inconsistent state. Transactions ensure all four operations happen together or none do.

### Webhook Integration Patterns

Webhooks allow external systems to react to JewelMatrix events. When registering a webhook, you provide a URL and an event type ("task:completed"). When that event occurs, JewelMatrix sends an HTTP POST to the webhook URL with event data. The external system processes it—maybe updating a CRM, sending Slack notifications, or triggering workflows.

Webhooks must be reliable despite network failures. JewelMatrix implements retry logic with exponential backoff—first retry after 1 minute, next after 2 minutes, then 4 minutes. After a configurable number of retries (3 by default), the webhook is disabled. This prevents hammering a broken endpoint forever.
...
Brute-forcing rules is difficult because they're stateless—every check must pass independently. Rules can't be bypassed by exploiting race conditions or timing attacks because there's no application state to corrupt.
...
Tokens contain claims—information about the user (UID, email, custom claims like role). These claims are cryptographically signed so users can't forge tokens claiming to be admin if they're just an employee.
...
**Injection Attacks**: Although Firestore is NoSQL, the principle applies—never construct queries using user input directly. Instead, validate user input against a whitelist before using it.
...
**Insecure Direct Object References**: Users might guess or brute-force object IDs. JewelMatrix prevents this by always verifying permissions—even if a user guesses the correct task ID, they can't access it without proper authorization.
...
**Batch Operations**: Updating 100 tasks individually is 100 write operations. Batching those updates into a single batch commit is 1 operation, reducing cost and latency by 100x. The downside is atomicity—if the batch partially fails, some documents update and others don't. For operations that don't require all-or-nothing semantics, batching is always preferred.

### Caching Strategy

Caching reduces database queries by storing recently-fetched data in memory. A user's profile is fetched when they log in, cached, and served from memory for subsequent requests. Cache entries expire (TTL—Time To Live) after a duration (5 minutes). If a user's profile changes, cached versions expire naturally after TTL, or manually invalidated immediately if consistency is critical.

The caching decision is: "Can this data be stale?" User profile can be 5 minutes stale—that's acceptable. A task status can't be stale—if the user just completed a task, they see the completion immediately, not 5 minutes later. For consistency-critical data, caching is minimal or combined with real-time listeners.
...
**Lazy Loading**: Images and components are loaded on-demand. Non-critical images are lazy-loaded—they load only when approaching visibility, reducing initial page load. Skeletons display while content loads, showing UI structure before data arrives.

**Memoization**: React components re-render when props or state change. A TaskCard component rendering thousands of tasks would re-render all of them if parent state changes, even if individual task data hasn't changed. Memoization caches the rendered output—if props haven't changed, the cached output is used without re-rendering.
...
1. **Don't fetch all data**
   ```typescript
   // ❌ WRONG: Fetches every task in database
...
5. **Don't query without pagination**
   ```typescript
   // ❌ WRONG: Memory explosion with 10K users
   const users = await db.collection('users').getDocs();
   
   // ✅ RIGHT: Paginate results
   const pageSize = 50;
