...
import { Bell, Search, Menu, X, ChevronDown, LogOut, User, Settings, Flag } from 'lucide-react'
import { useState, useEffect, useRef } from 'react'
import { getInitials, getAvatarColor, cn } from '@/lib/utils'
import { useAuth } from '@/contexts/AuthContext'
import { getHeaderConfig, getDefaultHeaderConfig } from '@/lib/headerService';
...
export default function Header({
  ...
  const themeColors = getThemeColors(headerConfig?.theme || 'indian')
        ...
        <div className="flex items-center space-x-6">
          <Link href="/dashboard" className="flex items-center space-x-3 group">
            ...
            <span className={`text-xl font-bold bg-gradient-to-r ${themeColors.primary} bg-clip-text text-transparent hidden sm:inline`}>
              {headerConfig?.title || customTitle}
            </span>
          </Link>
              ...
              <div className="absolute right-0 mt-3 w-80 bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 z-50 animate-scale-in">
                ...
                <div className="max-h-96 overflow-y-auto">
                  {notifications.slice(0, 5).map((notification) => (
                    <div
                      key={notification.id}
                      className={`p-4 border-b border-gray-100/50 hover:bg-gradient-to-r hover:from-orange-50/30 hover:to-red-50/30 cursor-pointer transition-all duration-200 ${!notification.read ? 'bg-gradient-to-r from-blue-50/50 to-purple-50/50' : ''}`}>
                ...
                <div className="p-4 border-t border-gray-200/50 text-center bg-gradient-to-r from-gray-50/50 to-white/50 rounded-b-2xl">
                  <button className="text-sm text-orange-600 hover:text-orange-700 font-semibold hover:bg-orange-50 px-3 py-1 rounded-lg transition-colors">
                ...
                <div className="absolute right-0 mt-3 w-56 bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 z-50 animate-scale-in">
                  ...
                  <div className="py-2">
                    <Link
                      ...
                      className="flex items-center space-x-3 w-full px-4 py-3 text-sm text-gray-700 hover:bg-gradient-to-r hover:from-orange-50/50 hover:to-red-50/50 transition-all duration-200 rounded-lg mx-2"
                    >
