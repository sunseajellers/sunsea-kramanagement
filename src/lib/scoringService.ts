...
export class ScoringService {
    ...
    static async generateWeeklyReport(
        ...
        return {
            ...
            workNotDoneRate: weekTasks.length > 0 ? Math.round(((weekTasks.length - weekTasks.filter(t => t.status === 'completed').length) / weekTasks.length) * 100) : 0,
            delayRate: weekTasks.length > 0 ? Math.round((delayedTasks / weekTasks.length) * 100) : 0,
            score: overallScore,
            breakdown: {
                completionScore,
                timelinessScore,
